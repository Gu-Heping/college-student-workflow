...
- **Install / update** of this skill must not modify the learning vault.
- Any workflow that writes files must **inspect Git status first** and must not auto-commit unless the user explicitly asks.
- Any text destined for a **public** GitHub Issue / PR review / comment must pass privacy checks first (`prepare_github_issue.py --stdin` / `--check-only`, or `sanitize_and_post.py`).
