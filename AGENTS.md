...
- Optional dependencies from `requirements.txt` (`pdfplumber`, `pypdf`, `pymupdf`, `python-docx`, `openpyxl`, `python-pptx`, `mineru-open-sdk`) are only needed for the document-import feature (`student-os/scripts/materials_convert.py`). The startup update script installs them.
...
- Gotchas:
  ...
  - `--method api` requires `MINERU_TOKEN`/`MINERU_API_TOKEN` (CLI, process env, or skill/cwd `.env`) and fails without it. Content-aware `--method auto` prefers MinerU when useful, but degrades to local converters or index sidecars when no token is available. Forced `--force-strategy mineru-api|ocr` also errors without a token. Optional `pandoc` improves DOCX routing.
