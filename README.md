...
```text
请用 student-os 为这门课构建期末复习资料包。目标学生假定没听过课、没做过作业、需要短时间速成。先读仓库里的 exam-prep gold standard，然后亲自打开课本/讲义/往年卷/答案，先手写 1 页高价值题型解析 gold page 和 1 份试卷精析样板，自己做 reader audit 并修改到可读。不要一次性批量生成整包，不要用脚本或 state JSON 拼正文；脚本只用于初始化、列来源、记录状态和做 tripwire 机械检查。样板通过后再逐批扩展。
```

题型解析质量要求：

```text
写题型解析前必须先完成对应 type dossier / 题型备课卡，但 dossier 只是备课卡，不是正文生成器。每页要像辅导讲义：先讲课本知识点和符号，再给 30 秒速记、2 分钟下笔模板、方法选择、常见变式、完整往年卷例题、完整解析、不重复的往年卷自测题和自测答案。例题和自测题必须全部选自往年卷 paper-card 引用（例如 2024-final.json#一），正文里要放可做的完整题面，不能只有来源链接；例题区和自测区不能重复同一道题；真题不足时标 quality: needs-review 并写清“证据不足，需人工补充”，不要自编题、模拟题或改编题。完成前必须亲自抽读入口页、题型页、例题、自测答案并报告 reader audit；`issue_count: 0` 只表示机械 tripwire 通过，不表示资料好用。
```
