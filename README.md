...
```text
请用 student-os 为这门课构建期末复习资料包。目标学生假定没听过课、没做过作业、需要短时间速成。不要只做机械题型统计，也不要一次性批量生成空泛文件；先盘点课本/讲义/往年卷/答案，写 quality-standard.md 和 source-map.json，然后先做 1 份试卷精析样板和 1 页题型解析样板并通过 gold-sample 检查。样板通过后，再逐卷精析 v0、生成题目卡、做跨卷题型/原题复现/趋势分析、回填精析 v1、生成 type dossier、题型解析和四层备考包。脚本只用于初始化任务、保存证据索引和做机械验收，真正内容理解由 AI 完成。
```

题型解析质量要求：

```text
写题型解析前必须先完成对应 type dossier / 题型备课卡。每页要像辅导讲义：先讲课本知识点和符号，再给 30 秒速记、2 分钟下笔模板、方法选择、常见变式、完整往年卷例题、完整解析、不重复的往年卷自测题和自测答案。例题和自测题必须全部选自往年卷 paper-card 引用（例如 2024-final.json#一），正文里要放可做的完整题面，不能只有来源链接；例题区和自测区不能重复同一道题；真题不足时标 quality: needs-review 并写清“证据不足，需人工补充”，不要自编题、模拟题或改编题。
```
