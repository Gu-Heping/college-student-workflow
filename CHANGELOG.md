...
- 为 `student-os` 新增统一的 `token_loader.py`：按 CLI → 进程环境变量 → skill 根目录 `.env` → cwd `.env` 加载 `MINERU_TOKEN` / `MINERU_API_TOKEN`，并提供 `.env.example`；安装/更新时保留 skill 本地 `.env`，脚手架默认忽略真实 `.env`。
