...
| **DSH native tools** | `integrations/dsh/`：本地 Cordis plugin，提供 `student_os_inspect` / `student_os_group_changes` / `student_os_frontmatter`，复用 Python core；`scripts/bootstrap_dsh.py` 可生成 project-local overlay；官方 API/package/bootstrap smoke 已通过，真实 DSH CLI overlay/runtime 仍需在已安装 DSH 的机器上验证 |
